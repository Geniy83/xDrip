 ...
 * Modified NSEmulatorReceiver for Syai
 */


public class SyaiAppReceiver extends BroadcastReceiver {

    private static final String TAG = "Syai";
    ...
    public void onReceive(final Context context, final Intent intent) {
        new Thread() {
            @Override
            public void run() {
                PowerManager.WakeLock wl = JoH.getWakeLock("syai-receiver", 60000);
                synchronized (lock) {
                    try {

                        Log.d(TAG, "Syai onReceiver: " + intent.getAction());
                        JoH.benchmark(null);
                        // check source
                        if (prefs == null)
                        ...
                        switch (action) {
                            case Intents.SYAI_TAG_APP:

                                // in future this could have its own data source perhaps instead of follower
                                if (!Home.get_follower() && DexCollectionType.getDexCollectionType() != DexCollectionType.SyaiAppReceiver &&
                                        !Pref.getBooleanDefaultFalse("external_blukon_algorithm")) {
                                    Log.e(TAG, "Received Syai data but we are not a follower or Syai receiver");
                                    return;
                                }
                                ...
                                Log.d(TAG, "Receiving Syai broadcast");
    ...
    public static BgReading bgReadingInsertFromData(long timestamp, double sgv, double slope, boolean do_notification) {
        Log.d(TAG, "Syai bgReadingInsertFromData called timestamp = " + timestamp + " bg = " + sgv + " time =" + JoH.dateTimeText(timestamp));
